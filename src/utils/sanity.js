...
export async function getKeywordPaginated(keyword, skip = 0, limit = 24) {
  const last = skip + limit;
  return client.fetch(`{"items": *[_type == "watches" && name match "${keyword}*"][${skip}...${last}],
  "total": count(*[_type == "watches" && name match "${keyword}*"])}`);
}
